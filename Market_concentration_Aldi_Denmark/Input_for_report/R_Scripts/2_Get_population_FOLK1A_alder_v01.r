...
Pop_data <- add_column(Pop_data, tmp = (as.matrix(select(Pop_data, ends_with("år")))%*%matrix(as.numeric(ages[,1])))[,1])

stop()

Pop_data <- Pop_data %>% 
                select(Id, Område, tmp, `I alt`) %>% 
                add_column(Alder = Pop_data$tmp/Pop_data$`I alt`) %>% 
                select(-c(tmp,`I alt`))

saveRDS(Pop_data, file = paste0("Input/Population_Age.rds"))
